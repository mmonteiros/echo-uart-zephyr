...
static struct bt_uuid_128 uppercase = BT_UUID_INIT_128(0x01, 0x23, 0x45, 0x67, 0x89, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x00, 0x00);
static struct bt_uuid_128 receive_data = BT_UUID_INIT_128(0x01, 0x23, 0x45, 0x67, 0x89, 0x01, 0x02, 0x03,0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0xFF, 0x00);
static struct bt_uuid_128 notifys = BT_UUID_INIT_128(0x01, 0x23, 0x45, 0x67, 0x89, 0x01, 0x02, 0x03,0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0xFF, 0x11);
static struct bt_uuid *uuid = &uppercase.uuid;
static void init_scan(void);
static struct bt_conn *connect_default;
...
static volatile size_t notifications;
...
static void gatt_subscribe(void)
{
	...
	err = bt_gatt_subscribe(connect_default, &sub_params);
	if (err < 0) {
		...
		struct bt_gatt_discover_params *params)
{
	...
	if (params->type == BT_GATT_DISCOVER_PRIMARY &&
	    bt_uuid_cmp(params->uuid, &uppercase.uuid) == 0) {
		...
		if (bt_uuid_cmp(chrc->uuid, &receive_data.uuid) == 0) {
			printk("Found rvd_chrc\n");
			chrc_handle = chrc->value_handle;
		}else if (bt_uuid_cmp(chrc->uuid, &notifys.uuid) == 0) {
...
static void gatt_discover(void)
{
	...
	err = bt_gatt_discover(connect_default, &discover_params);
	if (err != 0) {
		printk("Discover failed(err %d)\n", err);
	}
...
static void gatt_write(uint16_t handle, char* chrc_data)
{
	...
	err = bt_gatt_write(connect_default, &write_params);
	if (err != 0) {
			 ...
			 struct net_buf_simple *ad)
{
	...
	err = bt_conn_le_create(addr, BT_CONN_LE_CREATE_CONN,
				BT_LE_CONN_PARAM_DEFAULT, &connect_default);
	if (err) {
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (err) {
		...
		bt_conn_unref(connect_default);
		connect_default = NULL;
		init_scan();
		return;
	}
	if (conn != connect_default) {
		return;
	}
	printk("Connected: %s\n", addr);
}
...
void main(void)
{
	...
	if (error) {
		printk("Failed to initi Bluetooth (error %d)\n", error);
		return;
	}
	printk("Started Bluetooth\n");
	init_scan();
	read_input();
}