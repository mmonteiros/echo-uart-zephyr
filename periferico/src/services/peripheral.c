...
static ssize_t write_test_chrc(struct bt_conn *conn,const struct bt_gatt_attr *attr,const void *buf, uint16_t len,uint16_t offset, uint8_t flags)
{
...
{ 
		...
		BT_GATT_CHARACTERISTIC(&ble_uart_receive_data.uuid, BT_GATT_CHRC_WRITE_WITHOUT_RESP,BT_GATT_PERM_WRITE, NULL, write_test_chrc, NULL),
...
int ble_uart_service_transmit(const uint8_t *buffer, size_t len) 
{
    ...
    string[len] = '\0';
    return conn ? bt_gatt_notify(conn,&attrs[2],string,len): (-1);
}